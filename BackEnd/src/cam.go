...
func main() {
	dev := flag.String("d", "/dev/video2", "video device to use")
	...
	single := flag.Bool("m", true, "single image http mode, default mjpeg video")
...
FMT:
	...
	for {
		...
		if len(frame) != 0 {
			...
			fr++
			if *fps {
				if d := time.Since(start); d > time.Second*1 {
